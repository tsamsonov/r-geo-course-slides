...
```{r}
ifelse(lengths > 20, 'Длинная', 'Короткая')
```
...
Фактор — категориальный вектор со словарем допустимых значений:
```{r}
...
Извлечение элементов фрейма данных — это гибрид возможностей матрицы и списка
